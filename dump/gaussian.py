...
class VAE(nn.Module):
    def __init__(self, latent_dim, input_dim, channels, hidden_channels: list=[8, 16, 32]):
        ...
        self.input_dim = input_dim
        self.log_var = torch.nn.parameter.Parameter(torch.tensor(0.0).to(device))
    ...
    def forward(self, x, save_latent = False):
        ...
        #std = 0.05
        log_like = 1/(2 * var) * nn.functional.mse_loss(decode_mu, x.flatten(
            start_dim=1, end_dim=-1), reduction="none") + 1/2 * torch.log(2 * np.pi * var)
        ...
        tqdm.write(
            f"ELBO: {elbo.item()}, Reconstruction error: {reconstruction_error.item()}, Regularizer: {regularizer.item()}, std: {var.item()}")
